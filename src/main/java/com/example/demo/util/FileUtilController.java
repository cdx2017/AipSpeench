...
import com.fuzamei.bonuspoint.util.FastDFSClientUtil;
...
public class FileUtilController {
    ...
    public ResponseVO uploadImage(@RequestAttribute("token") Token token, MultipartFile[] images) {
        ...
        if (images != null && images.length > 0) {
            ...
            for (MultipartFile image : images) {
                try {
                    String url = FastDFSClientUtil.uploadFile(image.getBytes(), image.getOriginalFilename());
                    if (url != null) {
                        if (srcUrls.length() != 0) {
                            url = "," + url;
                        }
    ...
    public ResponseVO uploadAPK(@RequestAttribute("token") Token token, MultipartFile file) {
        ...
        try {
            String url = FastDFSClientUtil.uploadFile(file.getBytes(), file.getOriginalFilename());
            if (url == null) {
